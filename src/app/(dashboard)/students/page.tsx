      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="ghost" size="sm"><i className="bi bi-file-earmark-arrow-up" />Import</Button>
            <Button variant="primary" size="sm"><i className="bi bi-person-plus-fill" />Add Student</Button>
          </>
        }
      />
      ...
      {isEmpty ? (
        ...
        <>
          <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard icon="bi bi-people-fill"       color="blue"   value="1,248" label="Total Enrolled" compact />
            <StatCard icon="bi bi-person-check-fill" color="green"  value="1,201" label="Active"         compact />
            <StatCard icon="bi bi-person-plus-fill"  color="orange" value="47"    label="New This Term"  compact />
          ...
          <Card className="mb-5">
            <div className="flex flex-wrap items-end gap-2 p-4">
              <div className="relative min-w-[180px] flex-1">
                ...
                <input type="text" placeholder="Search by name, ID, class…"
          ...
          <Card>
            <CardHeader title="All Students" subtitle="1,248 enrolled"
              action={
                <>
                  <Button variant="ghost"   size="sm" className="px-2.5"><i className="bi bi-table" /></Button>
                  <Button variant="primary" size="sm" className="px-2.5"><i className="bi bi-grid" /></Button>
                </>
              }
            />
            <Table>
              <THead>
                ...
                <TH>Gender</TH><TH>Attendance</TH><TH>Fees</TH><TH>Status</TH><TH>Action</TH>
              </THead>
              <TBody>
                {searchEmpty ? (
                  <EmptyTableRow colSpan={9} icon="bi-search" message="No students match your search" />
                ) : (
                  STUDENTS.map((s, i) => (
                    <TR key={s.id}>
                      <TD><input type="checkbox" /></TD>
                      <TD><NameCell name={s.name} sub={s.id} index={i} /></TD>
                      <TD className="text-xs text-t2 whitespace-nowrap">{s.id}</TD>
                      <TD><Tag>{s.cls}</Tag></TD>
                      <TD><Badge color={s.gender === "Female" ? "pink" : "blue"}>{s.gender}</Badge></TD>
                      <TD className="min-w-[110px]"><StatBar pct={s.attendance} color={attendanceColor(s.attendance)} /></TD>
                      <TD><Badge color={s.fee === "Paid" ? "green" : s.fee === "Partial" ? "orange" : "red"}>{s.fee}</Badge></TD>
            ...
            {!searchEmpty && (
              <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border-light px-4 py-3">
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" size="sm"><i className="bi bi-chevron-left" /></Button>
                  <Button variant="primary" size="sm">1</Button>
                  <Button variant="ghost" size="sm">2</Button>
                  <Button variant="ghost" size="sm">3</Button>
                  <span className="self-center px-1 text-t3">…</span>
                  <Button variant="ghost" size="sm">125</Button>
                  <Button variant="ghost" size="sm"><i className="bi bi-chevron-right" /></Button>
                </div>
              </div>
            )}
          </Card>
