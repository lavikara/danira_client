      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="primary" size="sm"><i className="bi bi-check2-square" />Mark Attendance</Button>
          </>
        }
      />
      ...
      {isEmpty ? (
        ...
        <>
          <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard icon="bi bi-people-fill"   color="blue"   value="1,141" label="Present Today" compact />
            <StatCard icon="bi bi-person-x-fill" color="red"    value="68"    label="Absent Today"  compact />
            <StatCard icon="bi bi-clock-history" color="orange" value="39"    label="Late Arrivals" compact />
            <StatCard icon="bi bi-graph-up"      color="green"  value="91.4%" label="Overall Rate"  compact />
          </div>
                ...
                <div className="flex flex-col gap-3">
                  {ATTENDANCE_BY_CLASS.map((c) => (
                    <div key={c.name} className="flex items-center gap-3">
                      <span className="w-[54px] shrink-0 text-[12px] font-semibold text-t2">{c.name}</span>
            ...
            <Card>
              ...
              <CardBody>
                <AppChart type="line" height={210} data={{
                  ...
                  datasets: [{ data: ATTENDANCE_TREND_30D, borderColor: "#2563EB", backgroundColor: "rgba(37,99,235,.08)", fill: true, tension: 0.35, borderWidth: 2, pointRadius: 0 }],
          ...
          <Card>
            <CardHeader title="Live Attendance Register" subtitle="Real-time check-ins"
              action={
                <>
            ...
            <Table>
              <THead>
                <TH>Student</TH><TH>Adm. No.</TH><TH>Class</TH><TH>Time In</TH><TH>Status</TH>
              </THead>
              <TBody>
                {ATTENDANCE_REGISTER.length === 0 ? (
                  <EmptyTableRow colSpan={5} icon="bi-calendar-x" message="No attendance recorded yet" />
                ) : (
                  ATTENDANCE_REGISTER.map((r, i) => (
                    <TR key={r.id}>
                      ...
                      <TD><Badge color={r.status === "Present" ? "green" : r.status === "Late" ? "orange" : "red"}>{r.status}</Badge></TD>
                    </TR>
                  ))
                )}
              </TBody>
