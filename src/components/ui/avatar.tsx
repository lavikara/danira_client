...
export function NameCell({ name, sub, index = 0 }: { name: string; sub?: string; index?: number }) {
  return (
    <div className="flex items-center gap-2.5">
      <Avatar name={name} index={index} />
      <div>
        <div style={{ color: 'var(--t1)' }} className="text-[13px] font-semibold whitespace-nowrap">
          {name}
        </div>
        {sub && (
